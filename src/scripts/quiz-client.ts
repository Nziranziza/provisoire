...
function initQuiz(root: HTMLElement) {
  ...
  const pageCurrentEls = Array.from(root.querySelectorAll<HTMLElement>("[data-page-current], [data-page-current-bottom]"));
  const pageTotalEls = Array.from(root.querySelectorAll<HTMLElement>("[data-page-total], [data-page-total-bottom]"));
  ...
  function updateResultCard(item: HTMLElement, data: QuestionPayload) {
    ...
    if (statusEl) {
      statusEl.classList.remove(...RESULT_CARD.correct.status, ...RESULT_CARD.incorrect.status);
    ...
    if (iconEl) {
      iconEl.classList.remove(...RESULT_CARD.correct.icon, ...RESULT_CARD.incorrect.icon);
      iconEl.classList.add(...style.icon);
      iconEl.textContent = style.iconText;
    }
  }

  function showResultCard(item: HTMLElement, data: QuestionPayload, isCorrect: boolean) {
    const card = item.querySelector<HTMLElement>("[data-result-card]");
    if (!card) return;

    card.classList.remove("hidden", ...RESULT_CARD.correct.card, ...RESULT_CARD.incorrect.card);
    card.classList.add(...(isCorrect ? RESULT_CARD.correct.card : RESULT_CARD.incorrect.card));
    card.hidden = false;
  ...
  function updateNavigationState() {
    ...
    const pageComplete = pageItems.length > 0 && answeredCount === pageItems.length;

    if (prevPageButton) prevPageButton.disabled = currentPage === 1;
    if (nextPageButton) nextPageButton.disabled = currentPage >= totalPages || !pageComplete;
  }

  function updateScoreboard() {
    const pageItems = currentPageItems();
    const answered = pageItems.filter((item) => item.dataset.state).length;
    const correct = pageItems.filter((item) => item.dataset.state === "correct").length;
  ...
  function applyCategoryFilter(value: string) {
    filteredItems = items.filter((item) => value === "all" || item.dataset.categoryId === value);
    currentPage = 1;
    setSearchError("");
    renderPage();
  }
  ...
  function goToQuestionNumber(rawValue: string) {
    ...
    const target = items.find((item) => Number(item.dataset.questionIndex) + 1 === number);
    if (!target) {
  ...
  items.forEach((item) => {
    const data: QuestionPayload = JSON.parse(item.dataset.payload || "{}");
    const optionButtons = Array.from(item.querySelectorAll<HTMLButtonElement>(".quiz-option"));
    ...
    optionButtons.forEach((btn, idx) => {
      btn.addEventListener("click", () => {
        ...
        item.classList.add(...(isCorrect ? LEFT_BORDER.correct : LEFT_BORDER.incorrect));
